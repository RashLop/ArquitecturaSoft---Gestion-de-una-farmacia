...
namespace ProyectoArqSoft.Validaciones
{
    public class ClienteValidacion : IValidacion<Cliente>
    {
        ...
        public Validacion Validar(Cliente cliente)
        {
            ...
            if (!EsTextoValido(cliente.Nombre, 3, 45))
            ...
            if (!EsTextoValido(cliente.ApellidoMaterno, 3, 20))
                return new Validacion(false, "El apellido materno debe tener entre 3 y 20 caracteres y solo letras");

            if (!EsTextoValido(cliente.ApellidoPaterno, 3, 20))
                return new Validacion(false, "El apellido paterno debe tener entre 3 y 20 caracteres y solo letras");
            ...
            if (edad < 18 || edad > 80)
                return new Validacion(false, "La fecha de nacimiento debe corresponder a una edad entre 18 y 80 anos");

            if (!string.IsNullOrWhiteSpace(cliente.Telefono))
            {
                if (cliente.Telefono.Length > 8)
                    return new Validacion(false, "El telefono no puede superar 8 caracteres");
