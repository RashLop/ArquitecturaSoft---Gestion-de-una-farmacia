...
//                 return Validacion.Fail("El n˙mero de carnet es obligatorio");

//             if (ci.Contains(' '))
//                 return Validacion.Fail("El n˙mero de carnet no debe contener espacios");

//             if (!Regex.IsMatch(ci, @"^\d{5,10}(-\d+[A-Za-z])?$"))
//                 return Validacion.Fail("El CI debe tener de 5 a 10 dÌgitos y el formato correcto (ej: 1234567 o 1234567-1A)");
...
//                 return Validacion.Fail("La extensiÛn del CI es obligatoria");

//             if (!ExtensionesValidas.Contains(ciExtencion))
//                 return Validacion.Fail("La extensiÛn del CI no es v·lida");
...
//                 return Validacion.Fail("El telÈfono es obligatorio");

//             if (!Regex.IsMatch(telefono, @"^\d{8}$"))
//                 return Validacion.Fail("El telÈfono debe tener exactamente 8 dÌgitos");
...
//                 && valor.Length >= minimo
//                 && valor.Length <= maximo
//                 && Regex.IsMatch(valor, @"^[a-zA-Z·ÈÌÛ˙¡…Õ”⁄Ò—\s]+$");
//         }
//     }
...
//         {
//             string patronValido = @"^([a-zA-Z·ÈÌÛ˙¡…Õ”⁄Ò—\s]+|\d+|\d+-\d[A-Z])$";

//             return Regex.IsMatch(filtro, patronValido)
//                 ? null
//                 : Validacion.Fail("Criterio de b˙squeda inv·lido");
//         }
//     }
// }
